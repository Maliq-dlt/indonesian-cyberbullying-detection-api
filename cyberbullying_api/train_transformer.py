...
if min_class_count >= 2:
    train_texts, val_texts, train_labels, val_labels = train_test_split(
        df["text_clean"].tolist(), labels, test_size=0.15, random_state=42, stratify=stratify_key.tolist()
    )
else:
    train_texts, val_texts, train_labels, val_labels = train_test_split(
        df["text_clean"].tolist(), labels, test_size=0.15, random_state=42
    )
...
model = AutoModelForSequenceClassification.from_pretrained(
    MODEL_NAME, num_labels=2, problem_type="multi_label_classification"
)
model.to(device)
...
training_args_dict = {
    ...
    "per_device_train_batch_size": int(
        os.getenv("PER_DEVICE_TRAIN_BATCH_SIZE", "8" if not torch.cuda.is_available() else "16")
    ),
    "per_device_eval_batch_size": int(
        os.getenv("PER_DEVICE_EVAL_BATCH_SIZE", "8" if not torch.cuda.is_available() else "16")
    ),
...
def export_fine_tuned_to_onnx(model_dir, output_dir):
    ...
    def robust_infer_shapes_path(model_path, output_path, *args, **kwargs):
        try:
            ...
            print(
                f"[ONNX Warning] Shape inference gagal ({err}). Fallback bypass shape inference: {model_path} -> {output_path}"
            )
    ...
    slugs = ["nahiar_hatespeech-abusive-xlm-roberta-v1", "fine_tuned_transformer"]
