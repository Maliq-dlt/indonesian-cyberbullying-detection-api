...
def scrape_x_tweets_playwright(query: str, max_tweets: int = 20) -> List[str]:
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                page.wait_for_selector("[data-testid='tweetText']", timeout=7000)
            except Exception:
                pass
...
def scrape_x_tweets(query: str, max_tweets: int = 20) -> Tuple[List[str], bool]:
    """
    ...
    if PLAYWRIGHT_AVAILABLE and os.path.exists(COOKIES_X_PATH):
        tweets = scrape_x_tweets_playwright(query, max_tweets)
        if tweets:
